...
def recalibrate_scores(weibull_model, test_features, test_softmax, num_classes, alpharank=10, distance_type='eucos'):
    test_features = test_features.permute(1, 2, 0).detach().numpy()
    test_softmax = test_softmax.permute(1, 2, 0).detach().numpy()
    test_features = test_features.reshape(test_features.shape[0] * test_features.shape[1],
                                          test_features.shape[2])
    test_softmax = test_softmax.reshape(test_softmax.shape[0] * test_softmax.shape[1],
                                        test_softmax.shape[2])
...
# ---------------------------------------------------------------------------------
def openmax(weibull_model, pixel_feat, softmax_feat, alpharank = 7, num_classes = 7, distance_type = 'euclidean'):
    ranked_list = softmax_feat.argsort()[::-1]
    ...
    # Pass the recalibrated fc8 scores for the image into openmax
    openmax_probs = compute_openmax_probability(openmax_fc8, openmax_score_u, num_classes)
